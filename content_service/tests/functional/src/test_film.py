...
@pytest.mark.parametrize(
    ...
    [
        ({"page_size": 70, "page_number": 1}, {"status": HTTPStatus.OK, "length": 61}),
        ({"page_size": 50, "page_number": 1}, {"status": HTTPStatus.OK, "length": 50}),
        (
            {"page_size": 70, "page_number": 2},
            {"status": HTTPStatus.NOT_FOUND, "length": 1},
        ),
        ({"page_size": 10, "page_number": 1}, {"status": HTTPStatus.OK, "length": 10}),
        (
...
async def test_list_films(
    films_data, es_write_data, make_get_request, query_data, expected_answer
):
    ...
    if status == HTTPStatus.OK:
        ...
        for transmit in films_data:
            if transmit["_id"] in films_dict.keys():
                assert (
                    transmit["_source"]["title"]
                    == films_dict[transmit["_source"]["id"]]["title"]
                )
                assert (
                    transmit["_source"]["imdb_rating"]
                    == films_dict[transmit["_source"]["id"]]["imdb_rating"]
                )
...
@pytest.mark.parametrize(
    "query_data, expected_answer",
    [({"id": test_settings.es_film_id_field}, {"status": HTTPStatus.OK, "length": 8})],
)
@pytest.mark.asyncio
async def test_film_redis(
    ...
    url = test_settings.service_url + f"/api/v1/films/{query_data['id']}"
    response = await make_get_request(url, None)
    await delete_from_es(test_settings.es_film_index, query_data["id"])
