...
async def test_person_by_id(
    persons_data, es_write_data, make_get_request, query_data, expected_answer
):
        ...
        if status == HTTPStatus.NOT_FOUND:
            ...
            for transmit in persons_data:
                if transmit["_id"] in persons_dict.keys():
                    assert (
                        transmit["_source"]["full_name"]
                        == persons_dict[transmit["_id"]]["full_name"]
                    )
                    for films_transmit, films_recieve in zip(
                        transmit["_source"]["films"],
...
@pytest.mark.asyncio
async def test_search_persons_redis(
    es_write_data,
    request_by_id,
    request_search,
    delete_from_es,
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"id": test_settings.es_person_id_field, "page_size": 6, "page_number": 1},
        ...
        (
            {"id": test_settings.es_person_id_field, "page_size": -5, "page_number": 1},
...
async def test_person_films(
    ...
    url = test_settings.service_url + f"/api/v1/persons/{query_data['id']}/film"
    query = {
    ...
    if response.status == HTTPStatus.OK:
        ...
        for films_transmit, films_recieve in zip(
            data_films[query_data["id"]]["films"], films_dict.values()
        ):
            assert films_transmit["id"] == films_recieve["uuid"]
