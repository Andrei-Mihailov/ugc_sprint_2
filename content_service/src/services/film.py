...
class FilmService(BaseService):
    ...
    async def get_all_films(
        self, sort_by: str, page_number: int, page_size: int, genre: Union[str, None]
    ) -> Union["list[Film]", None]:
        if sort_by[0] == "-":
            sort = [{sort_by[1:]: "desc"}]
        else:
            sort = [{sort_by: "asc"}]
        if genre is None:
            ...
            query_str = {
                "nested": {"path": "genres", "query": {"match": {"genres.name": genre}}}
            }
